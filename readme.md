...
Запустить `setup.exe` нужной версии и дождаться установки. Если окно установки не появляется, вручную запустить файл `MSI`-файл для вашей ОС (зачастую это будет `amd64`).

> [!IMPORTANT]
> После установки **_необходимо_** перезагрузить систему во избежание ошибок.

> [!NOTE]
> Чтобы стандартная раскладка не мешалась в списке выбора языков, можно удалить её в `Настройки > Время и язык > Язык и регион > Английский (США) > Меню три точки > Языковые параметры > Раскладки`.

> [!NOTE]
> Чтобы раскладка была доступна на экране логина, заходим в `Административные языковые параметры > Копировать параметры`.

> [!NOTE]
> Записи в реестре, которые определяют доступные раскладки хранятся по адресу `Computer\HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Keyboard Layouts\`
